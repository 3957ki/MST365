...
# 콜백 정의
# ... (기존 handle_callback 및 run_with_callback 함수 그대로)
...
async def run_test(scenarios: List[dict]):
    async with MultiServerMCPClient(
            {
                "playwright": {"url": "http://localhost:8005/sse", "transport": "sse"}
            }
    ) as client:
...
if __name__ == "__main__":
    ...
    # JSON 구조 지원: {'title': ..., 'scenarios': [...]} 또는 리스트 직접
    ...
    if isinstance(data, dict) and "scenarios" in data:
        ...
        raise RuntimeError(f"지원하지 않는 JSON 포맷: {type(data)}")
