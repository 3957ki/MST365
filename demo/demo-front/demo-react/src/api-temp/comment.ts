...
export async function createComment(
  ...
  const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/v1/boards/${boardId}/comments`, {
    method: "POST",
    headers: {
...
export async function updateComment(
  ...
  const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/v1/boards/${boardId}/comments/${commentId}`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
      "Authorization": `Bearer ${token}`,
