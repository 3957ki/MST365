...
export default function MyPage() {
  ...
  // 사용자 댓글 목록 상태 추가
  const [userComments, setUserComments] = useState<UserCommentItem[] | null>(
    null
  );
  const [isUserCommentsLoading, setIsUserCommentsLoading] = useState(true);
  const [userCommentsError, setUserCommentsError] = useState<string | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const performFetchPosts = async () => {
      ...
      try {
        ...
        if (isMounted) {
          setUserPostsError(
            error.message || "게시물을 불러오는 데 실패했습니다."
          );
          setUserPosts(null);
        }
      } finally {
      ...
      try {
        ...
        if (isMounted) {
          setUserCommentsError(
            error.message || "댓글을 불러오는 데 실패했습니다."
          );
    ...
    <div className="container mx-auto p-8">
      ...
      {userPostsError && (
        <p className="text-center text-red-500 my-4">
          게시글 목록 로딩 오류: {userPostsError}
        </p>
      )}
      {/* 로딩 완료 및 에러 없을 때 UserPostsList 렌더링 */}
      {!isUserPostsLoading && !userPostsError && (
