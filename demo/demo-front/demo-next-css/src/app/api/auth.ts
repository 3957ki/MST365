...
// 에러 발생 시 공통적으로 반환되는 에러 구조
interface ApiErrorResponse {
  error: string;
  details?: string; // details는 선택적일 수 있음
}

// --- 토큰 관련 유틸리티 함수 (저장, 조회, 삭제 등) ---
...
interface GetUserPostsResponse {
  message: string;
  data: UserPostItem[]; // pagination 필드는 선택적이므로 타입에 포함하지 않거나 optional로 추가 가능
}
...
interface GetUserCommentsResponse {
  message: string;
  data: UserCommentItem[]; // pagination 필드는 선택적이므로 타입에 포함하지 않거나 optional로 추가 가능
}
