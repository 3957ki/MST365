...
export class MCPClient {
  ...
  private async isChromeBrowserInstalled(): Promise<boolean> {
    try {
      ...
      // 윈도우 호스트의 Chrome 확인
      try {
        const { stdout: winBrowserPath } = await exec(
          'cmd.exe /c "where chrome.exe"'
        );
        if (winBrowserPath.trim()) {
          log(
            LogLevel.INFO,
            `Windows 호스트에서 Chrome 발견: ${winBrowserPath.trim()}`
          );
          // 윈도우 Chrome이 있으면 WSL에서도 사용 가능
          return true;
        }
      } catch (error) {
        log(LogLevel.DEBUG, 'Windows 호스트의 Chrome 확인 실패:', error);
      }

      log(LogLevel.INFO, 'Chrome/Chromium이 설치되어 있지 않습니다.');
      return false;
    } catch (error) {
  ...
  private async installChromeBrowser(): Promise<boolean> {
    try {
      log(LogLevel.INFO, 'WSL Ubuntu에 Chrome 브라우저를 설치합니다...');
      console.log('Chrome 브라우저 설치 중...');

      // 필요한 패키지 업데이트 및 설치
      await exec('sudo apt-get update');
      await exec('sudo apt-get install -y wget apt-transport-https');

      // Chrome repo 추가 및 설치 (Ubuntu 24.04 대응)
      try {
        // 구글 크롬 저장소 추가
        await exec(
        ...
        await exec('sudo apt-get update');
        await exec('sudo apt-get install -y google-chrome-stable');
      } catch (chromeError) {
        log(
          LogLevel.WARN,
        ...
        try {
          await exec('sudo apt-get install -y chromium-browser');
        } catch (chromiumError) {
          log(LogLevel.ERROR, 'Chromium 설치 실패:', chromiumError);

          // 마지막 대안: 스냅 패키지로 Chromium 설치
          try {
            await exec('sudo apt-get install -y snapd');
            await exec('sudo snap install chromium');
            return true;
          } catch (snapError) {
            log(LogLevel.ERROR, '모든 브라우저 설치 시도 실패:', snapError);
            return false;
          }
        }
      }
      ...
      log(LogLevel.ERROR, 'Chrome 브라우저 설치 실패:', error);
      console.error('Chrome 브라우저 설치 실패:', error);
      return false;
    }
  }
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      // 한글 로케일이 설치되어 있는지 확인
      const { stdout: availableLocales } = await exec('locale -a');
      const hasKoreanLocale =
        availableLocales.includes('ko_KR') ||
        availableLocales.includes('ko_KR.UTF-8');

      if (!hasKoreanLocale) {
        log(
          LogLevel.INFO,
          '한글 로케일이 설치되어 있지 않습니다. 설치를 시도합니다...'
        );
        console.log('한글 로케일 설치 중...');

        // 한글 로케일 설치
        await exec('sudo apt-get update');
        await exec('sudo apt-get install -y language-pack-ko');
        await exec('sudo locale-gen ko_KR.UTF-8');
        await exec('sudo update-locale LANG=ko_KR.UTF-8');

        log(LogLevel.INFO, '한글 로케일 설치 완료');
        console.log('한글 로케일 설치 완료');
      }

      // 한글 로케일 설정 후
      await this.installKoreanFonts();
      ...
      log(LogLevel.WARN, '한글 로케일 설정 중 오류:', error);
      console.warn('한글 로케일 설정 중 오류가 발생했습니다.');
    }
    ...
    if (!isChromiumInstalled) {
      ...
      const installSuccess = await this.installChromeBrowser();

      if (!installSuccess) {
        const errorMsg =
          'Chrome/Chromium 설치에 실패했습니다. MCP 서버를 시작할 수 없습니다.';
    ...
    // WSL 환경에서는 DISPLAY 환경 변수 설정 확인
    if (process.platform === 'linux' && process.env.WSL_DISTRO_NAME) {
      log(LogLevel.INFO, 'WSL 환경 감지됨, DISPLAY 설정 확인...');

      if (!process.env.DISPLAY) {
        log(LogLevel.WARN, 'WSL에서 DISPLAY 환경 변수가 설정되지 않았습니다.');
        console.warn(
          'WSL에서 GUI 애플리케이션을 실행하려면 DISPLAY 환경 변수 설정이 필요할 수 있습니다.'
        );
        // process.env.DISPLAY = ':0'; // 기본값 설정 (상황에 따라 다를 수 있음)
      }
    }
